...
class TasNetMethod(Method):
    def __init__(self, run_sampler:bool,scenario:GWScenario,logger,config:MethodConfig,start_from_chekpt):
        ...
        self.method_type  = Method_type.TASNET
        self.singleLikel1 = SingleSignalMethod(True, scenario, logger,config,start_from_chekpt = False)
        self.singleLikel2 = SingleSignalMethod(True, scenario, logger,config,start_from_chekpt = False)
        self.singleLikel1.setNameExtra("1")
        self.singleLikel2.setNameExtra("2")
    ...
    def generateSamples(self):
        ...
        self.singleLikel1.overideLikelihood(ifos_A)
        self.singleLikel2.overideLikelihood(ifos_B)
        
        self.singleLikel1.generateSamples()
        resultsSampleA = copy.deepcopy(self.singleSampler.results["waveFormA"])
        
        self.singleLikel2.generateSamples()
        resultsSampleB = copy.deepcopy(self.singleSampler.results["waveFormA"])
    ...
    def getResdiualIfos(self):
        ...
        splitter = self.getSplittedWaveParams()
        ifos_A = []
        ifos_B = []
        ...
        for i,ifo in enumerate(self.scenario.ifos):
            ...
            waveForms = [torch.squeeze(s).detach().cpu().numpy() for s in splitter(strain)] 
            ...
            # 0.04s, if ever far away satelites are used this effect will need to be solved.
    ...
    def sampleAmplitudes(self,waveForms,ifo):
        ...
        result = bilby.run_sampler(
            ...
            thin_by    = 10,
            resume     = False,
            outdir     = "logs/log_ET_dynesty_"+ self.method_type.code + self.nameExtra + "_Amplitude",
            label      = "ml_2d",
            )
        return result
