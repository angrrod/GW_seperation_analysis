...
import os
from Pipeline.Pipeline_type import Pipeline_type
...
def Main(useJoint = True,onlyCorr = True):
    #build scenario
    scenario,logger,plot_dir,data_dir = setUpLoggerScenario()
    ...
    for parameter_name in ['mass_ratio', 'dec', 'luminosity_distance', 'chirp_mass', 'geocent_time', 'phase']:
        fig_cor = plot_joint_parameter_corner(posteriors = posteriors,parameter = parameter_name,copula_space = None)
        path_0 = os.path.join(plot_dir, f"_correlation_{parameter_name}.png")
        fig_cor.savefig(path_0, dpi=200)
        fig_cor_1 = plot_joint_parameter_corner(posteriors = posteriors,parameter = parameter_name,copula_space = "gaussian")
        path_1 = os.path.join(plot_dir, f"correlation_gauss_{parameter_name}.png")
        fig_cor_1.savefig(path_1, dpi=200)
        fig_cor_2 = plot_joint_parameter_corner(posteriors = posteriors,parameter = parameter_name,copula_space = "uniform")
        path_2 = os.path.join(plot_dir, f"correlation_uniform_{parameter_name}.png")
        fig_cor_2.savefig(path_2, dpi=200)
...
def plot_joint_parameter_corner(
    ...
    copula_space: str | None = None,
):
    """
    Plot the joint distribution of parameter_A and parameter_B.

    Parameters
    ----------
    copula_space:
        None
            Plot the original physical parameters.

        "uniform"
            Plot the empirical copula:
                u_A = Fhat_A(parameter_A)
                u_B = Fhat_B(parameter_B)

        "gaussian"
            Plot Gaussianized empirical-copula coordinates:
                z_A = Phi^{-1}(u_A)
                z_B = Phi^{-1}(u_B)

            This is the useful representation for testing whether
            a Gaussian / elliptical copula is appropriate.
    """
    ...
    if parameter not in df_A.columns:
        raise KeyError(
            f"{parameter!r} not found in {waveform_a_key}"
        )

    if parameter not in df_B.columns:
        raise KeyError(
            f"{parameter!r} not found in {waveform_b_key}"
        )

    if len(df_A) != len(df_B):
        raise ValueError(
            f"Cannot make joint plot: "
            f"{waveform_a_key} has {len(df_A)} samples, "
            f"but {waveform_b_key} has {len(df_B)} samples."
        )

    if strict_index_check and not df_A.index.equals(df_B.index):
        raise ValueError(
            f"{waveform_a_key} and {waveform_b_key} "
            "do not have matching indices. "
            "If row order is still meaningful, call with "
            "strict_index_check=False."
        )

    samples = np.column_stack(
        [
            df_A[parameter].to_numpy(dtype=float),
            df_B[parameter].to_numpy(dtype=float),
        ]
    )
    ...
    # ---------------------------------------------------------
    if copula_space is not None:

        n = len(samples)

        if n == 0:
            raise ValueError(
                "No finite samples remain after filtering."
            )

        # Pseudo-observations.
        #
        # Using (rank - 0.5) / n keeps u strictly inside (0, 1),
        # which is necessary before norm.ppf().
        u_A = (
            rankdata(samples[:, 0], method="average") - 0.5
        ) / n

        u_B = (
            rankdata(samples[:, 1], method="average") - 0.5
        ) / n

        if copula_space == "uniform":
            ...
            labels = [
                rf"$\hat F_A({parameter}_A)$",
                rf"$\hat F_B({parameter}_B)$",
            ]

            # Physical-space truth values no longer apply.
            truths = None

        elif copula_space == "gaussian":
            ...
            labels = [
                rf"$\Phi^{{-1}}(\hat F_A({parameter}_A))$",
                rf"$\Phi^{{-1}}(\hat F_B({parameter}_B))$",
            ]
